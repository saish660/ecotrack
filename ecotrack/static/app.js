...
class EcoTrackApp {
  ...
  updateDashboardUI(data) {
    ...
    if (carbonElement) {
      carbonElement.textContent = `${data.carbon_footprint.toFixed(1)} kg`;
    }
  ...
  initializeDailyHabitChecklist() {
    ...
    if (!form || !list) return;
    // Load habits from localStorage or default
    const defaultHabits = [
      { id: 1, text: "Use reusable water bottle" },
      { id: 2, text: "Take public transport" },
      { id: 3, text: "Turn off lights when leaving room" },
    ];
    const habits =
  ...
  initializeEcosystem3D() {
    ...
    for (let i = 0; i < animalCount; i++) {
      if (Math.random() < 0.5) {
        ...
        scene.add(rabbit);
        animals.push({ mesh: rabbit, type: 'rabbit', baseY: rabbit.position.y, phase: Math.random() * Math.PI * 2 });
      }
    }
    // --- More land details ---
